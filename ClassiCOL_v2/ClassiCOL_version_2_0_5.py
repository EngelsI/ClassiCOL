    ...
    for taxa in taxonomic_groups.keys():
        ...
        for protein,peps in protein_to_peptides.items():
            ...
            for a in animals_to_include:
                ...
                if aligned[0].count('-')+aligned[1].count('-')>0.2*len(str(reverse_sequences[temp[0]])) or aligned[0].count('-')+aligned[1].count('-')>0.2*len(str(reverse_sequences[protein])): #len(str(reverse_sequences[temp[0]]))<length_protein*0.9 or :#means protein too trunctated to consider or a totally different one
                    continue
