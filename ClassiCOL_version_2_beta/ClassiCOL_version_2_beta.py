...
def parse_input(path_to_output_file,file_extinct,mixture,sequences,taxonomy):
    ...
    df = pd.read_csv(path_to_output_file, header=0).fillna(0)
...
def ClassiCOL_mixture_analysis(
    ...
    path_to_output_file = file_extinct
    file_extinct = sample_path
    combined_file_output = {}
    missed_outcome = []
    print('Assigning taxonomic groups under each order level.')
    df, taxonomy, taxonomic_groups,species_in,rank_lca,restrict,taxonomy_missing = parse_input(path_to_output_file, file_extinct,mixture,sequences,taxonomy)
